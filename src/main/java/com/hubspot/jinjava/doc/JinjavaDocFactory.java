  ...
  private void addExpTests(JinjavaDoc doc) {
    for (ExpTest t : jinjava.getGlobalContext().getAllExpTests()) {
      ...
      if (docAnnotation == null) {
        ...
        doc.addExpTest(new JinjavaDocExpTest(t.getName(), "", "", false, new JinjavaDocParam[] {}, new JinjavaDocSnippet[] {}, Collections.<String, String>emptyMap()));
      }
  ...
  private void addFilterDocs(JinjavaDoc doc) {
    for (Filter f : jinjava.getGlobalContext().getAllFilters()) {
      ...
      if (docAnnotation == null) {
        ...
        doc.addFilter(new JinjavaDocFilter(f.getName(), "", "", false, new JinjavaDocParam[] {}, new JinjavaDocSnippet[] {}, Collections.<String, String>emptyMap()));
      }
      else if (!docAnnotation.hidden()) {
    ...
    for (ELFunctionDefinition fn : jinjava.getGlobalContext().getAllFunctions()) {
      if (StringUtils.isBlank(fn.getNamespace())) {
        ...
        if (docAnnotation == null) {
          ...
          doc.addFunction(new JinjavaDocFunction(fn.getLocalName(), "", "", false, new JinjavaDocParam[] {}, new JinjavaDocSnippet[] {}, Collections.<String, String>emptyMap()));
        }
        else if (!docAnnotation.hidden()) {
  ...
  private void addTagDocs(JinjavaDoc doc) {
    for (Tag t : jinjava.getGlobalContext().getAllTags()) {
      ...
      if (docAnnotation == null) {
        ...
        doc.addTag(new JinjavaDocTag(t.getName(), StringUtils.isBlank(t.getEndTagName()), "", "", false, new JinjavaDocParam[] {}, new JinjavaDocSnippet[] {}, Collections.<String, String>emptyMap()));
      }
      else if (!docAnnotation.hidden()) {
